...
async def run(query: str):
    ...
    async for chunk in manager.run(query):
        ...
        yield (
            status_text,
            full_report,
            gr.update(visible=False), # these matches the position/order of gr components defined in outputs[]
...
async def send_email_ui(email, report):
    yield gr.update(value="Sending...", interactive=False)
    
    await manager.send_email(email, report)

    yield gr.update(value="Report sent!", interactive=True)
...
with gr.Blocks(theme=gr.themes.Default(primary_hue="amber", text_size="lg")) as ui:
    gr.Markdown("# 🏄🏻 Deep Research Agent") #heading
    ...
    run_button.click(fn=run, inputs=query_textbox, outputs=[status, report, email_textbox, send_email_button])
    query_textbox.submit(fn=run, inputs=query_textbox, outputs=[status, report, email_textbox, send_email_button])

    clear_button.click(
        fn=lambda:("", gr.update(visible=False), gr.update(visible=False), ""),
        outputs=[query_textbox, email_textbox, send_email_button, report]
        )
    send_email_button.click(fn=send_email_ui, inputs=[email_textbox, report], outputs=[send_email_button])
    email_textbox.submit(fn=send_email_ui, inputs=[email_textbox, report], outputs=[send_email_button])
